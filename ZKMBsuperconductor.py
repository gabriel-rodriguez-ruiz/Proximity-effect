...
class ZKMBSuperconductivity():
    ...
    .. math ::
       ...
       + H.c.\right]
    """
    def __init__(self, t_x:complex, t_y:complex, mu:float, Delta_0:float, Delta_1: float,
    ...
    def _get_hopping_x(self):
        return 1/2*( -self.t_x*np.kron((tau_0+tau_z)/2, sigma_0)
                     + np.conj(self.t_x)*np.kron((tau_0-tau_z)/2, sigma_0)
                    +1j*self.Lambda*np.kron(tau_z, sigma_y)
                    +self.Delta_1*np.kron(tau_x, sigma_0) )
    def _get_hopping_y(self):
        return 1/2*( -self.t_y*np.kron((tau_0+tau_z)/2, sigma_0)
                    + self.t_y*np.kron((tau_0-tau_z)/2, sigma_0)
                    -1j*self.Lambda*np.kron(tau_z, sigma_x)
                    +self.Delta_1*np.kron(tau_x, sigma_0) )

class ZKMBSuperconductor(ZKMBSuperconductivity,
                                  Hamiltonian):
    def __init__(self, L_x:int, L_y: int, t_x:float, t_y:float, mu:float,
                 Delta_0:float, Delta_1:float, Lambda:float,
                 B_x:float, B_y:float, B_z:float):
        ZKMBSuperconductivity.__init__(self, t_x, t_y, mu, Delta_0, Delta_1, Lambda,
                                       B_x, B_y, B_z)
...
class ZKMBSuperconductorKYQYFulde(ZKMBSuperconductivity, Hamiltonian):
    r"""ZKM superconductor for a given k in the y direction and magnetic field.
    ...
    .. math::
        ...
        \xi_k = -2tcos(k) - \mu
    """
    def __init__(self,  k:float, q:float, L_x:int, t_x:float, t_y:float, mu:float,
                 Delta_0:np.ndarray, Delta_1:float, Lambda:float,
                 B_x:float, B_y:float, B_z:float):
        self.k = k
        self.q = q
        ZKMBSuperconductivity.__init__(self, t_x=t_x, t_y=t_y, mu=mu, Delta_0=Delta_0,
                                       Delta_1=Delta_1, Lambda=Lambda,
                                       B_x=B_x, B_y=B_y, B_z=B_z)
        Hamiltonian.__init__(self, L_x, 1, self._get_onsite,   # I pass a function in get_onsite
                             self._get_hopping_x(), np.zeros((4, 4)))
    def _get_onsite(self, i):
        return 1/2*( -2*self.t_y * np.cos(self.k)*np.cos(self.q) * np.kron(tau_z, sigma_0) 
                     +2*self.t_y * np.sin(self.k)*np.sin(self.q) * np.kron(tau_0, sigma_0)
                     - self.mu * np.kron(tau_z, sigma_0)
                + self.Delta_0[i] *np.kron( (tau_x+1j*tau_y)/2, sigma_0 )
                + self.Delta_0[i].conj()*np.kron( (tau_x-1j*tau_y)/2, sigma_0 )
                -2*self.Lambda*np.sin(self.k)*np.cos(self.q)*np.kron(tau_z, sigma_x)
                -2*self.Lambda*np.cos(self.k)*np.sin(self.q)*np.kron(tau_0, sigma_x)
                -self.B_x*np.kron(tau_0, sigma_x)
                -self.B_y*np.kron(tau_0, sigma_y)
                -self.B_z*np.kron(tau_0, sigma_z))
    def _get_hopping_x(self):
...
class ZKMBSuperconductorKXQX(ZKMBSuperconductivity, Hamiltonian):
    r"""ZKM superconductor for a given k in the x direction and magnetic field.
    ...
    .. math::
        ...
        \vec{c} = (c_{k,\uparrow}, c_{k,\downarrow},c^\dagger_{-k,\downarrow},
                   -c^\dagger_{-k,\uparrsow})^T
    
        \xi_k = -2tcos(k) - \mu
    """
    def __init__(self,  k:float, q: float, L_y:int, t:float, mu:float,
                 Delta_0:float, Delta_1:float, Lambda:float,
                 B_x:float, B_y:float, B_z:float):
        self.k = k
        self.q = q
        ...
        Hamiltonian.__init__(self, 1, L_y, self._get_onsite(),
                             np.zeros((4, 4)), self._get_hopping_y())
    def _get_onsite(self):
        return 1/2*( - 2*self.t * np.cos(self.k)*np.cos(self.q) * np.kron(tau_z, sigma_0) 
                     + 2*self.t * np.sin(self.k)*np.sin(self.q) * np.kron(tau_0, sigma_0)
                     - self.mu * np.kron(tau_z, sigma_0) -
                self.Delta_0*np.kron(tau_x, sigma_0) +
                +2*self.Lambda*np.sin(self.k)*np.sin(self.q) * np.kron(tau_z, sigma_y)
                +2*self.Lambda*np.sin(self.k)*np.sin(self.q) * np.kron(tau_0, sigma_y)
                -self.B_x*np.kron(tau_0, sigma_x)
                -self.B_y*np.kron(tau_0, sigma_y)
                -self.B_z*np.kron(tau_0, sigma_z))
    def _get_hopping_y(self):
...
class ZKMBSuperconductorFulde(ZKMBSuperconductivity,
                                  Hamiltonian):
    def __init__(self, L_x:int, L_y: int, t_x:float, t_y:float, mu:float,
                 Delta_0:np.ndarray, Delta_1:float, Lambda:float,
                 B_x:float, B_y:float, B_z:float):
        ZKMBSuperconductivity.__init__(self, t_y, mu, Delta_0, Delta_1, Lambda,
                                       B_x, B_y, B_z)
        Hamiltonian.__init__(self, L_x, L_y, self._get_onsite, 
                             self._get_hopping_x(),
                            self._get_hopping_y()) 
    def _get_onsite(self, j):
        return 1/2*(-self.mu*np.kron(tau_z, sigma_0)
                    + self.Delta_0[j]*np.kron( (tau_x+1j*tau_y)/2, sigma_0 )
                    + self.Delta_0[j].conj()*np.kron( (tau_x-1j*tau_y)/2, sigma_0 )
                    -self.B_x*np.kron(tau_0, sigma_x)
                    -self.B_y*np.kron(tau_0, sigma_y)
                    -self.B_z*np.kron(tau_0, sigma_z))
    def _get_hopping_y(self):
        return 1/2*( -self.t_y*np.kron((tau_0+tau_z)/2, sigma_0)
                    + np.conj(self.t_y)*np.kron((tau_0-tau_z)/2, sigma_0)
                    -1j*self.Lambda*np.kron(tau_z, sigma_x)
                    +self.Delta_1*np.kron(tau_x, sigma_0) )
    def _get_matrix(self):
        r"""
