    ...
    H_s = (
        -2*w_s*((np.cos(k_x)*np.cos(phi_x+q_x) + np.cos(k_y)*np.cos(phi_y+q_y))
               * np.kron(tau_z, sigma_0)
               - (np.sin(k_x)*np.sin(phi_x+q_x) + np.sin(k_y)*np.sin(phi_y+q_y))
               * np.kron(tau_0, sigma_0)) - mu_s * np.kron(tau_z, sigma_0)
        + 2*Lambda*(np.sin(k_x)*np.cos(phi_x+q_x) * np.kron(tau_z, sigma_y)
                    + np.cos(k_x)*np.sin(phi_x+q_x) * np.kron(tau_0, sigma_y)
                    - np.sin(k_y)*np.cos(phi_y+q_y) * np.kron(tau_z, sigma_x)
                    - np.cos(k_y)*np.sin(phi_y+q_y) * np.kron(tau_0, sigma_x))
