...
def integrate(B):
    ...
    n[0], n[1], n[2] = get_superconducting_density(L_x, L_y, w_s, w_S, mu,
                       Delta_s, Delta_S, B_x, B_y, B_x_S, B_y_S, Lambda, w_1, h)
    return n

if __name__ == "__main__":
    B_values = np.linspace(0, Delta_S/2, points)
    with multiprocessing.Pool(n_cores) as pool:
        results_pooled = pool.map(integrate, B_values)
    n_B_y = np.array(results_pooled)
    
    data_folder = Path("Data/")
    name = f"n_By_mu_{mu}_L={L_x}_h={np.round(h,4)}_B_y_in_({np.min(B_values)}-{np.round(np.max(B_values),3)})_Delta={Delta_S}_lambda={Lambda}_w_s={w_s}_w_S={w_S}_w_1={w_1}_points={points}_field_in_S={field_in_S}.npz"
